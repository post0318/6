...
규칙:
- 초기 편입: 시작일에 지수 값과 무관하게 포트폴리오의 30%를 주식으로 매수.
- 매수 진행 모드(buying_active): FG가 65 밑으로 처음 내려가면 켜짐. 이후 FG가 다시 65를
  넘어도 꺼지지 않고 유지된다("최초 65는 매수진행을 위한 트리거"). 매수 진행 모드가 켜져
  있는 동안, 매주 수요일마다 포트폴리오 비중의 5%p를 추가로 주식에 편입(현금->주식),
  100%에 도달하면 더 이상 매수하지 않는다.
- 전량매도: 어느 시점이든 FG가 75 이상이면 그날 종가로 보유 주식을 전량 매도하고 매수 진행
  모드를 끈다(다시 65 하회를 기다려야 재개).
- 급락 전량매수: 매수 진행 모드 중 FG가 25 밑으로 내려가면 그날 즉시 나머지 현금을 전부
  주식으로 편입(100%).
- 매수/매도는 모두 당일 종가 기준. 현금 보유 구간은 무이자(0%), 거래비용 없음.

가정(사용자 문구 중 "지수가 상승하면 매주 수요일 5%씩 매수한다"는 매수 진행 모드 동안의
상황 설명으로 해석했고, 65 하회 트리거와 별개의 추가 매수 조건으로는 넣지 않았다. 의도와
다르면 조정 필요).
"""
